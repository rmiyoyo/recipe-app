...
RSpec.describe 'recipes/show.html.erb', type: :feature do
  ...
  describe 'Testing integration specs for recipes show page' do
    before :each do
      @inventory = Inventory.create(user:, name: 'Home', description: 'Lorem')
      @recipe = Recipe.create(name: 'Lorem', preparation_time: 20, cooking_time: 10, description: 'Donec dapibus turpis a turpis consequat ornare.',
                              public: false, user:)
      food = Food.create(name: 'LoremLorem', measurement_unit: 'g', price: 20)
